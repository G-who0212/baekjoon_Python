...
while(box_to_move): # 무조건 가능
    ...
    for i in range(cN):
        for j in range(bN):
            if bCheck[j] == True: continue
            if cL[i] >= bL[j]: # 옮길 수 있는 경우
                bCheck[j] = True
                box_to_move.remove(bL[j])
                if(i == cN - 1): # 모든 크레인이 작동하여 옮긴 경우
                    ans += 1
                    check = True
                    ...
                    check = True
                break
            elif bL[j] == box_to_move[len(box_to_move) - 1]: # 어느 크레인에서, 옮겨야 하는 마지막 박스까지 확인했는데 못 옮긴 경우
                ans += 1
